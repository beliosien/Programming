...
public class RandomizedQueue<Item> implements Iterable<Item> {
    ...
    private class ReverseArrayIterator implements Iterator<Item> {
        ...
        ReverseArrayIterator() {
            StdRandom.shuffle(shuffleQueue, shuffleQueue.length - size, shuffleQueue.length);
        }
        ...
        public Item next() {
            if (i == 0) {
                ...
                int index = StdRandom.uniform(shuffleQueue.length - size, shuffleQueue.length);
                Item item = shuffleQueue[index];
                while (item == null) {
                    index = StdRandom.uniform(shuffleQueue.length - size, shuffleQueue.length);
    ...
    public Item dequeue() {
        if (this.isEmpty()) {
            ...
            int index = StdRandom.uniform(queue.length - size, queue.length);
            Item item = queue[index];
            while (item == null) {
                index = StdRandom.uniform(queue.length - size, queue.length);
                item = queue[index];
            }
            queue[index] = null;
            this.size--;
    ...
    public Item sample() {
        if (this.isEmpty()) {
            ...
            int index = StdRandom.uniform(this.queue.length - size, queue.length);
            Item item = queue[index];
            return item;
        }
    }
